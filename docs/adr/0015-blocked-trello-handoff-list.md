...
`--blocked` list name. If there is no blocked list but there is a review handoff list, the
generated prompt tells Codex to move blocked work to review so it leaves the active list.
...
* Good, because the blocked state is visible in the board lists.
...
* Bad, because blocked and reviewable work share the same list and must be distinguished by the
  Trello comment.
...
* Bad, because it does not make blocked work visible in the board lists by itself.
