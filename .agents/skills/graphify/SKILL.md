...
> **graphify needs no API key for this repository workflow. Never ask the user for one, and never block on one.** Code is extracted structurally (AST) with no LLM and no key at all. Semantic extraction for docs, papers, and images should use the active coding agent when the user asks for a full repository graph.

When semantic extraction is needed, use the host agent itself. On a host that dispatches subagents, dispatch them as written in Part B. On a host that runs the CLI directly in a terminal and cannot dispatch subagents, do not stall: a code-only corpus has no semantic work, so write the empty semantic file (Part B "Fast path") and continue to Part C; for a corpus with docs/papers/images, extract those inline yourself or explain that a full semantic graph needs an assistant-capable run.
