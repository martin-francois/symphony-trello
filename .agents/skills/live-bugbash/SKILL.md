...
- Installer, uninstaller, onboarding, cleanup, repair, and destructive local-data tests must set `HOME`, `XDG_*`, `SYMPHONY_HOME`, app, config, state, cache, workspace, prefix, bin, manifest, and env-file paths to run-owned locations. In real-integration runs, pass live auth through explicit auth-only credentials or minimal cleanup-required run-scoped auth copies; do not use the operator's normal homes to make live Trello/GitHub/Codex execution work.
