...
ORIGINAL_PATH="$PATH"
DEFAULT_VERSION="1.1.1" # x-release-please-version
...
write_install_context() {
  ...
  mkdir -p "$CONFIG_DIR" "$STATE_HOME" 2>/dev/null || return
  ...
  context_content="$(
    {
      ...
      printf 'install_format_version=2\n'
      printf 'layout_mode=%s\n' "$LAYOUT_MODE"
      printf 'home_logical=%s\n' "$HOME_LOGICAL"
      printf 'home_real=%s\n' "$HOME_REAL"
      ...
      printf 'systemd_service_path=%s\n' "$SYSTEMD_SERVICE_PATH"
      printf 'autostart_env_path=%s\n' "$AUTOSTART_ENV_PATH"
      printf 'microos_var_root=%s\n' "$MICROOS_VAR_ROOT"
      printf 'created_microos_var_root=%s\n' "$MICROOS_VAR_ROOT_CREATED"
      printf 'dry_run=%s\n' "$DRY_RUN"
      printf 'no_onboard=%s\n' "$NO_ONBOARD"
  ...
  local context_file
  for context_file in "$INSTALL_CONTEXT_FILE" "$CONFIG_INSTALL_CONTEXT_FILE"; do
    write_install_context_file "$context_file" "$context_content"
  done
  for context_file in "$(default_state_install_context_file)" "$(default_config_install_context_file)"; do
    stable_install_context_file_safe "$context_file" || continue
    write_install_context_file "$context_file" "$context_content"
  done
}

write_install_context_file() {
  local context_file="$1" context_content="$2"
  mkdir -p "$(path_parent "$context_file")" 2>/dev/null || return 0
  printf '%s\n' "$context_content" >"$context_file" || true
}
...
apply_install_context() {
  ...
  value="$(install_context_value "$context_file" home_logical || true)"
  [[ -n "$value" ]] && HOME_LOGICAL="$value"
...
current_groupname() {
  if [[ -n "${SYMPHONY_TRELLO_TEST_GROUP:-}" ]]; then
    printf '%s\n' "$SYMPHONY_TRELLO_TEST_GROUP"
  else
    id -gn
  fi
}
...
apply_generic_xdg_layout() {
  ...
  LAYOUT_MODE="${LAYOUT_MODE:-generic-xdg}"
  if [[ "$SYMPHONY_HOME_CONFIGURED" != true ]]; then
...
validate_app_paths() {
  ...
  for root in "$CONFIG_DIR" "$WORKSPACE_ROOT" "$STATE_HOME"; do
    ...
    if same_or_inside_path "$app_dir_resolved" "$root_resolved" || same_or_inside_path "$root_resolved" "$app_dir_resolved"; then
...
write_user_systemd_service() {
  local command_unit environment_file_unit path_unit
  command_unit="$(systemd_quote "$BIN_DIR/symphony-trello")"
  environment_file_unit="$(systemd_environment_file_path "$AUTOSTART_ENV_PATH")"
...
EnvironmentFile=-$environment_file_unit
...
prepare_microos_var_root() {
  ...
  [[ -n "$MICROOS_VAR_ROOT" ]] || return 0
  if path_has_symlink_component "$MICROOS_VAR_ROOT"; then
    echo "Recommended MicroOS data root must not contain symlinked path components: $MICROOS_VAR_ROOT" >&2
    exit 2
  fi
  if [[ -d "$MICROOS_VAR_ROOT" ]]; then
    if [[ ! -w "$MICROOS_VAR_ROOT" ]]; then
      echo "Recommended MicroOS data root is not writable: $MICROOS_VAR_ROOT" >&2
      echo "Run:" >&2
      echo "  $(microos_var_root_create_command)" >&2
      exit 2
    fi
    if ! microos_var_root_mode_safe; then
      echo "Recommended MicroOS data root must not be writable by group or other: $MICROOS_VAR_ROOT" >&2
      echo "Run:" >&2
      echo "  $(microos_var_root_create_command)" >&2
      exit 2
    fi
  ...
  echo
  echo "Detected MicroOS-like storage layout."
  ...
  if ! terminal_available; then
    echo "Create the recommended MicroOS data root, then rerun this installer:" >&2
    echo "  $(microos_var_root_create_command)" >&2
    exit 2
  fi
  local create_command create_root=false
  create_command="$(microos_var_root_create_command)"
  echo "Create command:"
  echo "  $create_command"
  prompt_yes_no_default_yes create_root "Create this directory now? [Y/n] "
  if [[ "$create_root" != true ]]; then
    echo "Create the recommended MicroOS data root, then rerun this installer:" >&2
    echo "  $create_command" >&2
    exit 2
  fi
  if ! privileged_command_available; then
    echo "Creating the MicroOS data root requires root privileges." >&2
    echo "Install sudo/doas, run as root, or run this command before rerunning the installer:" >&2
    echo "  $create_command" >&2
    exit 2
  fi
...
microos_var_root_mode_safe() {
  local mode group_digit other_digit
  mode="$(stat -c '%a' "$MICROOS_VAR_ROOT" 2>/dev/null)" || return 1
  mode="${mode: -3}"
...
microos_var_root_create_command() {
  local prefix username groupname
  prefix="$(privileged_command_prefix || true)"
  username="$(current_username)"
  groupname="$(current_groupname)"
  printf '%sinstall -d -m 0750 -o %s -g %s %s' \
    "$prefix" "$(shell_literal "$username")" "$(shell_literal "$groupname")" "$(shell_literal "$MICROOS_VAR_ROOT")"
}
...
privileged_command_prefix() {
  ...
  [[ "${SYMPHONY_TRELLO_TEST_NO_PRIVILEGE_HELPERS:-false}" == true ]] && return 1
  if need sudo; then
    printf 'sudo '
    return 0
  fi
...
privileged_command_available() {
  if [[ "${SYMPHONY_TRELLO_TEST_NO_PRIVILEGE_HELPERS:-false}" == true ]]; then
    [[ "$(effective_uid)" == "0" ]]
    return
  fi
  [[ "$(effective_uid)" == "0" ]] || need sudo || need doas
}
...
CONFIG_INSTALL_CONTEXT_FILE="$CONFIG_DIR/install-context.properties"
activate_managed_codex_path
activate_brew_openjdk
...
INSTALLED_APP_HOME=$APP_DIR_LITERAL
INSTALLED_CONFIG_DIR=$CONFIG_DIR_LITERAL
INSTALLED_WORKSPACE_ROOT=$WORKSPACE_ROOT_LITERAL
INSTALLED_STATE_HOME=$STATE_HOME_LITERAL
