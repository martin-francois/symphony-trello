...
- For PR handoff behavior, do not treat every non-green CI state as blocked. If CI fails because of
  the card's changes or current branch, or equivalent local checks fail, the card should stay active
  while Codex fixes it. If CI is pending or stale, the card should stay active while Codex waits,
  refreshes, or reruns checks. If CI is unavailable because of external quota/infrastructure limits,
  or failures are clearly unrelated to the card, handoff to human review is acceptable when
  equivalent local validation passed and the Trello comment records the caveat.
