...
- If checks cannot run because of external quota or infrastructure limits, or the failing checks are
  clearly unrelated to the current card, do not block handoff when equivalent local validation
  passed. Record the check caveat, the local commands used as evidence, and why the failure is
  unrelated or unavailable in the workpad and handoff comment.
